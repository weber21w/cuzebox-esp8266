...
static void  cu_avr_write_io(auint port, auint val)
{
 ...
 switch (port){
  ...
  case CU_IO_DDRA:
   ...
   cpu_state.iors[CU_IO_PINA] = t1|t0;
...
static auint cu_avr_read_io(auint port)
{
 ...
 switch (port){
  ...
  case CU_IO_UCSR0A: /* UART Rx/Tx ready status */
   cpu_state.iors[CU_IO_UCSR0A] &= ~((1<<UDRE0) | (1<<RXC0));//default: not ready to read or write
   cpu_state.iors[CU_IO_UCSR0A] |= cu_esp_uzebox_read_ready(cpu_state.cycle);
   cpu_state.iors[CU_IO_UCSR0A] |= cu_esp_uzebox_write_ready(cpu_state.cycle);
